    ...
    public static class UserDataAccess
    {
        public static void AddUser(User user)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                var dateOfBirthParameter = new SqlParameter("@BirthDate", SqlDbType.DateTime) {Value = user.BirthDate };
                command.Parameters.Add(dateOfBirthParameter);
                var emailAddressParameter = new SqlParameter("@Email", SqlDbType.VarChar, 50) {Value = user.Email };
